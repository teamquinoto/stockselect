...
function paintImportOrigen(){
  ...
  host.innerHTML = `
    ...
    ${terc?`<p class="hint" style="margin:-4px 0 10px;font-size:12px">In the next step you set <b>Ours</b> per line (the units you keep → stock). The owner's units are <b>never your stock and can't be sold</b> — they're only tracked (traceability) until you hand them over.</p>`:""}`;
...
function openImportOwnerPicker(anchor){
  ...
  const crear=(nombre)=>{
    nombre=(nombre||"").trim();
    if(!nombre){ toast("Type the owner's name first","warn"); return; }
  ...
  // Mini-form inline (dentro del popup) para tipear el nombre del nuevo owner.
  const showCreate=(prefill)=>{
    search.style.display="none";
    listEl.innerHTML=`
      <div style="padding:8px">
        <input class="inp" id="newOwnerName" placeholder="Owner name (client / local)" value="${esc(prefill||"")}" autocomplete="off" spellcheck="false">
        <div style="display:flex;gap:8px;margin-top:8px">
          <button type="button" class="btn primary sm" id="newOwnerOk" style="flex:1">Create</button>
          <button type="button" class="btn sm" id="newOwnerCancel">Cancel</button>
        </div>
        <p class="hint" style="font-size:11px;margin:8px 2px 0">Only the name is needed here. You can add address/email later under Customers.</p>
      </div>`;
    const inp=listEl.querySelector("#newOwnerName");
    listEl.querySelector("#newOwnerOk").onclick=()=>crear(inp.value);
    listEl.querySelector("#newOwnerCancel").onclick=()=>{ search.style.display=""; paint(search.value); search.focus(); };
    inp.onkeydown=(e)=>{ if(e.key==="Enter"){ e.preventDefault(); crear(inp.value); } if(e.key==="Escape"){ e.preventDefault(); e.stopPropagation(); search.style.display=""; paint(search.value); search.focus(); } };
  ...
  const paint=(q)=>{
    ...
    listEl.querySelectorAll("[data-pio]").forEach(it=> it.onclick=()=>{
      ...
      if(v==="__new"){ showCreate(search.value); return; }   // abre el mini-form (no exige tipear antes)
