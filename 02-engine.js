...
function transferStock(prod, origen, destino, cantidad, costoExtraUnit, obs){
  ...
  consumed.forEach(c=>{ if(!c.synthetic) fifoLayers(prod, destino).push({ id:uid(), fecha:new Date().toISOString(), cantidad:c.cantidad, costoUnit:round2(c.costoUnit + costoExtraUnit), ref:"from "+storeName(origen)+legTxt }); });
  if(isBucket(destino)){
